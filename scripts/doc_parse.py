...
def parse(lines: col.Iterable[str]) -> NodeTag:
    ...
    rx_initial_tag = re.compile(r'^(.*?)(#\s*<(/?)([A-Za-z0-9_\-]+)([^>]*)>)(.*)$')
    rx_subsequent_tag = re.compile(r'^(.*?)<(/?)([A-Za-z0-9_\-]+)([^>]*)>(.*)$')
    rx_tokens = re.compile(r'([a-zA-Z0-9_\-]+)="([^"]*)"|([a-zA-Z0-9_\-]+)')

    def _append_text(text: str):

        current = stack[-1]
        if not current.children or isinstance(current.children[-1], NodeTag):
            new_node = NodeText()
            all_texts.append(new_node)
            current.children.append(new_node)
        current.children[-1].content.append(text)

    def _process_tag(is_closing: str, tag_name: str, attr_str: str):
            ...
            if current_node.tag_name == tag_name:
                ...
                if attrs and attrs != current_node.kwargs:
                    raise ValueError(
                        ...
                        f"do not match opening tag kwargs {current_node.kwargs}"
                    )

                stack.pop()
            else:
                raise ValueError(f"Mismatched closing tag: </{tag_name}>. Expected </{current_node.tag_name}>")
        else:
    ...
    for line in lines:
        ...
        while remainder:
            ...
            text_before, is_closing, tag_name, attr_str, remainder = sub_match.groups()
