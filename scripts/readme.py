...
def _main() -> None:
    ...
    parser_reference = doc_patch.FuncParser.from_func(txt_reference)
    ...
    file_overview = dir_docs / 'index.md'
    file_rationale = dir_docs / 'rationale.md'
    file_examples = dir_docs / 'examples.md'
    file_reference = dir_docs / 'reference.md'

    file_to_parser = {
        file_readme: parser_readme,
        file_overview: parser_overview,
        file_rationale: parser_rationale,
        file_examples: parser_examples,
        file_reference: parser_reference,
    }

    for file, parser in file_to_parser.items():
        file.write_text(
            parser.render_str().replace('```gml', '```js'),
            encoding='utf-8'
        )
